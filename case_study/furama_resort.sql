create database furamma_management;
use furamma_management;
...
select * from nhan_vien where CHAR_LENGTH(ho_ten)<=15 and (ho_ten like 'H%' or ho_ten like  'T%' or ho_ten like 'K%');

-- 3.	Hiển thị thông tin của tất cả khách hàng có độ tuổi từ 18 đến 50 tuổi và có địa chỉ ở “Đà Nẵng” hoặc “Quảng Trị”.
select * from khach_hang where (datediff(curdate(),ngay_sinh)/365.25 between 18 and 50 ) and  (dia_chi like '%Đà Nẵng' or dia_chi like '%Quảng Trị');
-- 4.	Đếm xem tương ứng với mỗi khách hàng đã từng đặt phòng bao nhiêu lần. Kết quả hiển thị được sắp xếp tăng dần theo số lần đặt phòng của khách hàng. Chỉ đếm những khách hàng nào có Tên loại khách hàng là “Diamond”.
select ho_ten , count(h.ma_khach_hang) as so_lan_da_dat from khach_hang c 
join hop_dong h on h.ma_khach_hang = c.ma_khach_hang 
join loai_khach l on l.ma_loai_khach = c.ma_loai_khach
where l.ten_loai_khach = 'Diamond'
group by h.ma_khach_hang
order by so_lan_da_dat asc;
...
where dv.ma_dich_vu NOT IN (SELECT hd.ma_dich_vu FROM hop_dong as hd WHERE quarter(hd.ngay_lam_hop_dong) = 1 and year(hd.ngay_lam_hop_dong) = 2021);
...
select hd.ma_hop_dong, nv.ho_ten as ten_NV, kh.ho_ten as ten_KH, kh.sdt, dv.ten_dich_vu, count(hdct.ma_hop_dong_chi_tiet) as so_luong_dvdk,tien_dat_coc
from hop_dong hd
	...
	JOIN hop_dong_chi_tiet hdct ON hd.ma_hop_dong = hdct.ma_hop_dong
		...
		and ngay_lam_hop_dong between '2020-10-01' and '2020-12-31'
group by hd.ma_hop_dong; 	

-- 13.	Hiển thị thông tin các Dịch vụ đi kèm được sử dụng nhiều nhất bởi các Khách hàng đã đặt phòng. (Lưu ý là có thể có nhiều dịch vụ có số lần sử dụng nhiều như nhau).
select hdct.ma_dich_vu_di_kem, dvdk.ten_dich_vu_di_kem,sum(so_luong) as so_lan_da_su_dung
from dich_vu_di_kem dvdk 
join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
group by hdct.ma_dich_vu_di_kem
having sum(so_luong) = (select max(so_luong)
                       from (select sum(so_luong) as so_luong
                            from hop_dong_chi_tiet
                            group by  ma_dich_vu_di_kem) as thongKeDVDK);
...
from hop_dong_chi_tiet hdct
	...
	join loai_dich_vu ldv on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu
where dvdk.ma_dich_vu_di_kem = (select ma_dich_vu_di_kem 
								from hop_dong_chi_tiet
...
select nv.ma_nhan_vien, nv.ho_ten, td.ten_trinh_do, bp.ten_bo_phan, nv.sdt, nv.dia_chi  from nhan_vien nv
join hop_dong hd on hd.ma_nhan_vien = nv.ma_nhan_vien
left join trinh_do td on td.ma_trinh_do = nv.ma_nhan_vien
